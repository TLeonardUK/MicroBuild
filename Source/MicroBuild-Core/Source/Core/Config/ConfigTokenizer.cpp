		...
		for (unsigned int i = 0; i < m_tokens.size(); i++)
		{
			...
			if (tok.Type == TokenType::Include)
			{
				...
				if (!subPath.Exists())
				{
					...
					{
						for (int j = 0; j < (int)includePaths.size() && !subPath.Exists(); j++)
						{
							subPath = (includePaths[j] + m_tokens[i + 2].Literal);
						}
					}
				}
				...
				subIncludePaths.push_back(subPath.GetDirectory());
				subIncludePaths.insert(
					subIncludePaths.end(), 
					includePaths.begin(), includePaths.end()
				);

				if (!subTokenizer.Tokenize(subPath, subIncludePaths))
				{
					return false;
				}
