...
bool ConfigFile::ParseExpressionComparison(ConfigFileExpression& expression)
{
	...
	while (!EndOfTokens())
	{
		...
		if (nextTok.Type != TokenType::Less &&
			...
			nextTok.Type != TokenType::MatchEqual)
		{
			break;
		}
...
bool ConfigFile::Parse(
	...
	const std::vector<Platform::Path>& includePaths
)
{
	m_path = path;
	m_tokenIndex = 0;
	...
	if (!m_tokenizer.Tokenize(path, includePaths))
	{
		return false;
	}
	...
	std::string& result)
{
	...
	if (iter != m_groups.end())
	{
		...
		{
			for (auto viter = keyIter->second->Values.rbegin(); viter != keyIter->second->Values.rend(); viter++)
			{
				ConfigFileValue* value = *viter;
	...
	case TokenType::MatchEqual:
		{
			...
			result = Strings::IsMatch(lValueResult.Result, rValueResult.Result);
