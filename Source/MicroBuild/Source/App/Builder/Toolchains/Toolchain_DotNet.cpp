...
Toolchain_DotNet::Toolchain_DotNet(ProjectFile& file)
	: Toolchain(file)
{
...
Platform::Path Toolchain_DotNet::FindLibraryPath(const Platform::Path& lib)
{
	if (lib.IsAbsolute())
	{
		return lib;
...
void Toolchain_DotNet::GetCompileArguments(std::vector<BuilderFileInfo>& files, std::vector<std::string>& args)
{
...
bool Toolchain_DotNet::Archive(std::vector<BuilderFileInfo>& files, BuilderFileInfo& outputFile)
{
	...
	Log(LogSeverity::Fatal, "C# projects do not support static libraries.");
...
bool Toolchain_DotNet::Link(std::vector<BuilderFileInfo>& files, BuilderFileInfo& outputFile)
{
	...
	GetCompileArguments(files, arguments);

	Platform::Process process;
	if (!process.Open(m_compilerPath, Platform::Path::GetExecutablePath().GetDirectory(), arguments, true))
	{
		return false;
	}
	...
	for (auto& library : m_projectFile.Get_Libraries_Library())
	{
		...
		// Can't do dependency tracking on this library as we don't know where it is! We assume
		// ones that do not have absolute paths are system ones, which shouldn't be changing if we are
		// targeting the same framework.
