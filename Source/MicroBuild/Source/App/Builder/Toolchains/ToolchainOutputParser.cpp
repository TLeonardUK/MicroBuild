...
ToolchainOutputParser::ToolchainOutputParser()
{
    // This is a generic error format that all toolchains support. This just makes
    // it easier for plugins and tools to emit error codes that will always be picked
    // up during microbuild runs.

    // origin(Line): fatal/error/warning/message ErrorNumber: Text
    RegisterOutput(
        R"(^(.*)\((\d+)\): (fatal|error|warning|message): (.*)$)",
        {
...
{
    // origin(Line): fatal/error/warning/message: Text
    TestOutput(
        R"(Path\To\File.c(100): warning: Error description)",
        {
            "Path\\To\\File.c",
            "100",
            "warning",
...
{
    bool bReadingExtentedErrorMessage = false;
	...
	while (startOffset < input.size())
	{
        ...
        // If this line follows an error message and starts with a space or > then its part of 
        // of an extended error message.
        // todo
        if (bReadingExtentedErrorMessage)
        {
            if (line.size() > 0 && (line[0] == '>' || line[0] == ' ' || line[0] == '\t'))
            {
                extractedOutput[extractedOutput.size() - 1].Details.push_back(line);
            }       
            else
            {
                bReadingExtentedErrorMessage = false;
            }
        }
                ...
                if (line.find("Note: including file:") == std::string::npos)
                {
                    for (MatchType& matchType : m_matchTypes)
                    {
                        ...
                        if (std::regex_search(line, match, matchType.Pattern))
                        {
                            ...
                            bReadingExtentedErrorMessage = true;
                            break;
                        }
                    }
                }
