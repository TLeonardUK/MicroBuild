...
void XCode_ProjectFile::Write_PBXNativeTarget(
	...
	const std::string& prebuildPhaseId,
	const std::string& prelinkPhaseId,
	const std::string& postbuildPhaseId,
    std::vector<ProjectFile*>& dependencies,
...
void XCode_ProjectFile::Write_PBXShellScriptBuildPhase(
	PlistNode& root,
...
{
	PlistNode& targetNode = root.Node("%s", phaseId.c_str());
	targetNode.Node("isa").Value("PBXShellScriptBuildPhase");
	targetNode.Node("buildActionMask").Value("2147483647");
	targetNode.Array("files");
	targetNode.Array("inputPaths");
	targetNode.Node("name").Value("%s", phaseName.c_str());
	targetNode.Array("outputPaths");
	targetNode.Node("runOnlyForDeploymentPostprocessing").Value("0");
	targetNode.Node("shellPath").Value("/bin/sh");
	targetNode.Node("shellScript").Value("%s", Strings::Quoted(Strings::Join(commands, "\n")).c_str());
}
...
{ 
	...
	std::vector<std::string> prelinkCommands;
	...
	Write_PBXShellScriptBuildPhase(root, "Prebuild", prebuildPhaseId, prebuildCommands);
	Write_PBXShellScriptBuildPhase(root, "Prelink", prelinkPhaseId, prelinkCommands);
	Write_PBXShellScriptBuildPhase(root, "Postbuild", postbuildPhaseId, postbuildCommands);
}
...
)
{
	...
	std::string prebuildPhaseId =
		Strings::Uuid(24, { rootUuid, "PBXShellScriptBuildPhase", "prebuild" });
	std::string prelinkPhaseId =
		Strings::Uuid(24, { rootUuid, "PBXShellScriptBuildPhase", "prelink" });
	std::string postbuildPhaseId =
		Strings::Uuid(24, { rootUuid, "PBXShellScriptBuildPhase", "postbuild" });
	...
	Write_PBXNativeTarget(objectsNode, rootUuid, targetId,  targetConfigListId, projectFile.Get_Project_Name(), projectFile.Get_Project_OutputType(), sourceBuildPhaseId, frameworksPhaseId, resourcesPhaseId, prebuildPhaseId, prelinkPhaseId, postbuildPhaseId, dependencies, projectFile.Get_Project_Language());
	...
	Write_PBXShellScriptBuildPhase(objectsNode, prebuildPhaseId, prelinkPhaseId, postbuildPhaseId, buildMatrix);
