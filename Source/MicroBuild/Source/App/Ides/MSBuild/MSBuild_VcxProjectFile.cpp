...
)
{
	...
	for (auto matrix : buildMatrix)
	{
...

		{
			...
			case EOutputType::StaticLib:
				linkGroup.Node("SubSystem").Value("Console");
				linkGroup.Node("EntryPointSymbol").Value("");
				break;
			default:
				projectFile.ValidateError(
					"Output type '%s' is not valid for msbuild C++ projects.",
			...
			std::string warningSupression = "";

			for (auto warning : disabledWarnings)
			{
				warningSupression += Strings::Format("/ignore:%s ", warning.c_str());

			}

			linkGroup.Node("AdditionalOptions").Value("%s %s %%(AdditionalOptions)", 
				matrix.projectFile.Get_Build_LinkerArguments().c_str(), 
				warningSupression.c_str());
		}
