...
MSBuild_IbtProjectFile::MSBuild_IbtProjectFile(
...
bool MSBuild_IbtProjectFile::Generate(
...
)
{
	...
	for (auto matrix : buildMatrix)
	{
		...
		propertyGroup.Node("NMakeBuildCommandLine").Value("%s IbtBuild %s %s -c=%s -p=%s --silent", 
		...
		propertyGroup.Node("NMakeCleanCommandLine").Value("%s IbtClean %s %s -c=%s -p=%s --silent",
			Strings::Quoted("$(SolutionDir)\\" + relativeMicroBuildPath.ToString()).c_str(), 
			Strings::Quoted(workspaceFile.Get_Workspace_File().ToString()).c_str(),
			projectFile.Get_Project_Name().c_str(),
			matrix.config.c_str(),
		...
		propertyGroup.Node("NMakeReBuildCommandLine").Value("%s IbtBuild %s %s -c=%s -p=%s -r --silent",
			Strings::Quoted("$(SolutionDir)\\" + relativeMicroBuildPath.ToString()).c_str(), 
			Strings::Quoted(workspaceFile.Get_Workspace_File().ToString()).c_str(),
			projectFile.Get_Project_Name().c_str(),
			matrix.config.c_str(),
