...
IbtBuildCommand::IbtBuildCommand(App* app)
	: m_app(app)
	, m_rebuild(false)
{
	SetName("ibtbuild");
	...
	SetDescription("Builds the given project that has been generated "
				   "using the internal build tool. Do not call directly, "
				   "use the Project.UseInternalBuildTool flag in your project files.");
...
bool IbtBuildCommand::Invoke(CommandLineParser* parser)
{
		...
		if (databaseFileLocation.Exists())
		{
			...
			if (databaseFile.Read())
			{
				...
				{					
					...
					if (builder.Build(*buildProjectFile, m_rebuild))
					{
						return true;
					}
				}
