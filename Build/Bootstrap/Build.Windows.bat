...
CALL msbuild MicroBuild.sln /m:%NUMBER_OF_PROCESSORS% /t:rebuild /p:Configuration=Shipping /p:Platform=x64
if %ERRORLEVEL% NEQ 0 GOTO MSBuildErrorHandler

echo Building x86 bootstrap binary ...
CALL msbuild MicroBuild.sln /m:%NUMBER_OF_PROCESSORS% /t:rebuild /p:Configuration=Shipping /p:Platform=Win32
if %ERRORLEVEL% NEQ 0 GOTO MSBuildErrorHandler
